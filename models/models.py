    ...
    def action_inventory_cancel(self):
        for rec in self:
            ...
            if rec._check_stock_account_installed():
                ...
                # cancel stock valuation
                stock_valuation_layer_ids = rec.sudo().mapped('move_ids').sudo().mapped('stock_valuation_layer_ids')
                if stock_valuation_layer_ids:
                    stock_valuation_layer_ids.sudo().unlink()
    ...
    def sh_cancel(self):
        if self.company_id.adj_operation_type == 'cancel':
            ...
            if self._check_stock_account_installed():
                ...
                # cancel stock valuation
                stock_valuation_layer_ids = self.sudo().mapped('move_ids').sudo().mapped('stock_valuation_layer_ids')
                if stock_valuation_layer_ids:
                    stock_valuation_layer_ids.sudo().unlink()
